...
def transform_color_spaces(image):
    ...
    ycbcr_image = cv2.cvtColor(image, cv2.COLOR_BGR2YCrCb) # OpenCV uses YCrCb
...
if process_button:
    ...
    if uploaded_file is None:
        ...
        if original_image is None:
            ...
            # --- Perbaikan Tampilan Saluran Individual ---
            st.markdown("### Citra Hasil Transformasi HSV (Hue, Saturation, Value)")
            h, s, v = cv2.split(hsv_image)
            col_hsv1, col_hsv2, col_hsv3 = st.columns(3)
            with col_hsv1:
                st.image(h, caption="Saluran Hue (HSV)", use_container_width=True, channels="GRAY")
            with col_hsv2:
                st.image(s, caption="Saluran Saturation (HSV)", use_container_width=True, channels="GRAY")
            with col_hsv3:
                st.image(v, caption="Saluran Value (HSV)", use_container_width=True, channels="GRAY")
            st.markdown("""
